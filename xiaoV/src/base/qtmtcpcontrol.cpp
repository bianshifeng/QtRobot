...
void QTmTcpControl::new_tcp_data()
{
    ...
    if( m_cmdDataSize <= m_cmdData.size() && m_cmdDataSize>10){
        ...
        }else if(0 == m_package.parse_json_cmd_object(strJson, TM_CMD_Wheel, jsVal )){
            write_ack(m_package.make_json_ack(true,TM_CMD_Wheel,"test"));
            emit signal_msg_cmd(TM_CMD_Wheel, jsVal);
        }

        m_cmdData.remove(0, m_cmdDataSize);
        m_cmdDataSize = 0;
    }
    if( m_cmdData.size() > 256 ){
        g_log("Buff is to long, clear it!");
        m_cmdData.clear();
    }
}
